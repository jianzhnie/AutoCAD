...
RewardMap = NewType('RewardMap', Dict[str, Tuple[Union[int, float],
                                                 Union[int, float]]])


def calc_quadratic(x: float,
                   a: float = 0.0,
                   b: float = 0.0,
                   c: float = 0.0) -> float:
    return a * x**2 + b * x + c
...
class Reward(ABC):
    ...
    def __init__(self, red_rewards: RewardMap,
                 blue_rewards: RewardMap) -> None:
